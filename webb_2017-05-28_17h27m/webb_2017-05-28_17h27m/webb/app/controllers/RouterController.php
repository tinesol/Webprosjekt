...
class RouterController extends Controller
{
    ...
    public function process($params, $db, $action = null)
    {
        ...
        $controllerClass = $this->dashesToPascalCase(array_shift($parsedUrl))
            . 'Controller';
        ...
        // Get action from URL
        if (!empty($parsedUrl[0])) {
            $this->action = $this->dashesToCamelCase(array_shift($parsedUrl));
        }

        // Calls the controller
        $this->controller->process($parsedUrl, $db, $this->action);
